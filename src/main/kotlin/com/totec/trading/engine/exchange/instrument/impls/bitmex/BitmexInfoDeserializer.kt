...
class BitmexInfoDeserializer @Inject constructor(override val currenciesAllowed: CurrenciesAllowed,
...
/*	private fun JsonReader<*>.readNullablePeriod(): Long {
	...
	override fun FastJsonReader.readObject(): BitmexInfo? {
		// We don't need startObject() because readSet() starts it for every object.
		// However, we need to do endObject()
		
		val info = BitmexInfo(System.currentTimeMillis())
		// var token: Byte = '{'.toByte()
		nextAttribute@ while (last != '}') { // should be ,
			
			val string = readProperty()
			when {
				string == "symbol"              -> {
						...
						is LongList -> {
							when {
								...
								info.isTimePeriod(string) -> {
									if (isStringOrCompleteNull()) {
										val timestamp = readTimestamp()
										val period = timestamp - periodOffset
										list.add(period)
									}
								}
								else                      -> {
